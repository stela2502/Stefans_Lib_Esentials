...
## populate functions
my $masterObj = "thisisreallyUnlikelyToMatchAnywhere";
for ( my $i = 0; $i < @file; $i ++ ) {
	if ( $file[$i] =~ m/definition\s*=\s*function\s*\(\s*([\w]+)/ ){
		$masterObj = $1;
		if ( $masterObj eq 'x' || lc($masterObj) eq "dataobj" ){
			## this is OK
		}
		else {
			$masterObj = "thisisreallyUnlikelyToMatchAnywhere";
		}
	}
	$file[$i] =~ s/$masterObj\$/$masterObj\@/g;
	print OUT $file[$i];
}
