...
=head2 calculate_and_return_from_column({ 'data_column' => 'a table column name or array of'}, function => sub { my ($data_table,@data) =@_; print join("_", @data);})

Like calculate_on_columns this function takes a data_column and a function reference.

But unlikely the calculate_on_columns the results from the function is not stored in the table itself.
The function decides what to do with it.

=cut

sub calculate_and_return_from_column {
	my ( $self, $hash ) = @_;
	unless ( defined $hash->{'data_column'} ) {
		Carp::confess(
			ref($self)
			  . ":calculate_and_return_from_column needs a 'data_column' and 'function' has entry at start"
		);
	}
	my @columns = $self->Header_Position( $hash->{'data_column'} );
	unless ( defined $columns[0] ) {
		Carp::confess("I do not know the column '$hash->{'data_column'}'\n");
	}
	for ( my $i = 0 ; $i < $self->Rows() ; $i++ ) {
		&{ $hash->{'function'} }
		  ( $self, @{ @{ $self->{'data'} }[$i] }[@columns] );
	}
	return $self;
}
...
sub calculate_on_columns {
	...
	Carp::confess($error) if ( $error =~ m/\w/ );
	return $self->calculate_and_return_from_column($hash)
	  unless ( defined $hash->{'target_column'} );
...
sub get_column_entries {
	my ( $self, $col_name ) = @_;
	$self->{'as_array'} ||= {};
	return $self->{'as_array'}->{$col_name}
	  if ( defined $self->{'as_array'}->{$col_name} );
...
=head2 set_value_for ($index_name, $index_value, $column_name, $value )
	...
	identifies the rows $self->using get_rowNumbers_4_columnName_and_Entry ( $index_name, $index_value ) and
	the columns by using $self->Header_Position($column_name)
=cut

sub set_value_for {
	my ( $self, $index_name, $index_value, $column_name, $value ) = @_;
	my @line_nr =
	  $self->get_rowNumbers_4_columnName_and_Entry( $index_name, $index_value );
	...
	foreach ( @{ $self->{'data'} }[@line_nr] ) {
		@$_[ $self->Header_Position($column_name) ] = $value;
	}
	return $self;
}
...
"Sorry, but we need a key 'column_name' in the hash that you gave me!"
	...
	if ( ref( $hash->{'colsub'} ) eq 'CODE' ) {
		$self->{'__HTML_column_mods__'}->{ $hash->{'column_name'} }->{'colsub'}
		  = $hash->{'colsub'};
	}
...
sub AsString {
	...
	foreach my $data ( @{ $self->{'data'} } ) {
		...
		$str .= join(
			$line_sep,
			map {
				if   ( defined $line[$_] ) { $line[$_] }
				else                       { $default_values[$_] }
			} 0 .. $#line
		) . "\n";
...
sub Add_2_Header {
	...
	$self->__col_format_is_string( $self->{'header_position'}->{$value}, 0 )
	  unless (
		$self->__col_format_is_string( $self->{'header_position'}->{$value} ) );
	...
	if ( defined $self->{'string_separator'} ) {
		...
		foreach ( split( "", $line ) ) {
			if ( $_ eq $self->{'string_separator'} ) {
				$in_string = !$in_string;
				if ($in_string) {
					$self->__col_format_is_string( $temp, $in_string )
					  unless ( $self->__col_format_is_string($temp) );
				}
...
sub merge_with_data_table {
	...
	unless ( defined @$keys_array[0] ) {
		...
		Carp::confess(
			    " Sorry, I could not use the predefined column titles '"
			  . join( "' '", @$keys_array )
